...
EXECUTION_INSTRUCTIONS = [
    ...
    "Use doc_paragraphs, doc_evidence, and doc_entity_attributes for markdown evidence and facts.",
    "Use doc_entity_attributes.<attribute_column>, doc_entity_attributes.record_anchor_name, doc_evidence.target_value, doc_evidence.evidence_text, and doc_paragraphs.paragraph_text exactly as aligned in semantic_plan; do not replace document evidence with unrelated same-named values.",
    "Use doc_entity_attributes for document entity filtering, joins, and aggregation. It has one row per document entity, so multiple document attributes on the same entity should be ordinary filters on doc_entity_attributes columns, not self-joins.",
    "For document lookup joins, prefer joining structured key/link/code fields to doc_entity_attributes.record_anchor_name or a specific doc_entity_attributes attribute column returned by join evidence.",
    ...
    "When a semantic_plan answer_column contains calculation instead of source_field, execute that complete calculation expression as a requested output metric. For COUNT_IF(condition) over joined document entities, count distinct doc_entity_attributes.record_anchor_name when that field is present so joins do not duplicate entities.",
    ...
    "Execute NOT_BETWEEN with a range object as `(field < lower OR field > upper)`, not as an AND of the two comparisons.",
    "Execute OR filter groups as parenthesized disjunctions over their ordinary filter conditions.",
    ...
    "When semantic_plan.group_by is present for tally/distinct/unique output, return one row per group instead of duplicate detail rows.",
